...
- フィールド名の修正
    ...
    - リクエストやレスポンスにもユニークキーの概念はある
        - プロパティのフィールド名はカラム名に合わせておくことが多い
        - 基本的に1つのリソースに対して1つのテーブルが特定されているべきであり、1つのエンドポイントに対するメソッドは、1つのテーブルからデータを撮ってくる(他と結合しない)ことが多い(そう設計されるべき)
        - よって返されるデータに対しidがユニークキーとして適切に機能する単位で返されるべきである
        - POSTは情報を全部返し、GETは必要最低限だけ返す、のような基準があるともいえる
- データベースモデル定義
    - user.py
    - weight_record.py
    - auth_token.py
